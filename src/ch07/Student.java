...
// extends 클래스
// 부모클래스의 필드 및 메소드를 상속받는다
// inheritance(상속): 프로그래밍언어에서 상속은 자식이 부모를 선택해서 물려 받음
// SE(SW공학) -> generalization(일반화) -> inheritance(상속) (일반화랑 상속이랑 같은말)
/*
    여러 개의 클래스를 구현하는데, 일반적인 속성(attribute)과 기능(operation)이 있더라
    그거를 부모 클래스로 만들고, 나머지 클래스들은 상속받고 필요한 것만 추가하자.
 */
// 공개된 클래스의 좋은 코드를 사용하기 위한 것?
// 상속받은 클래스를 사용하고 소스코드를 통해 내가 원하는 기능을 추가?

public class Student extends Person {
    private String studentId;   // 학번추가

    // Student는 name, age 필드, introduce 메서드를 그냥 사용할 수 있습니다.

    // 빈 생성자는 자동으로 상속받은
    // 하지만 매개변수가 있는 생성자는 상속할 떄 별도로 선언해줘야함
